...
def _make_rows(result):
    ...
    for entry in result['results']:
        row = dict(entry, median=None, p95=None)
        if entry['status'] == 'measured':
            samples = np.array(entry['round_elapsed_ns'], dtype='float64')
            samples /= repetitions
            row['median'], row['p95'] = np.percentile(samples, [50, 95])
        row['tooltip'] = (
            f'{entry["name"]}: {entry["status"]}\n'
            f'Median: {_format_number(row["median"])} ns/call\n'
            f'p95: {_format_number(row["p95"])} ns/call\n'
...
class TimingChart(QtWidgets.QWidget):
    ...
    def paintEvent(self, event):
        ...
        maximum = max(row['p95'] for row in self._rows) or 1
        ...
        for index, row in enumerate(self._rows):
            ...
            median_x = plot.left() + plot.width() * row['median'] / maximum
            p95_x = plot.left() + plot.width() * row['p95'] / maximum
            painter.fillRect(
