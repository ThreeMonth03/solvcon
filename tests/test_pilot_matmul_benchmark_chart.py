    ...
    return [
        {
            ...
            'timing': _summary(660.0, 700.0, 19.8),
        },
        {
            'name': 'naive',
            'kind': 'solvcon_route',
            ...
            'correctness': {'correct': True},
            'timing': _summary(620.0, 680.0, 31.0),
        },
        {
            'name': 'blas_gemm',
            'kind': 'solvcon_route',
            'packing': {},
            'correctness': {'correct': True},
            'timing': _summary(500.0, 560.0, 20.0),
        },
        {
            'name': 'broken',
            'kind': 'solvcon_route',
        ...
        },
        {
            ...
            'packing': None,
            'correctness': {'correct': True},
            'timing': _summary(400.0, 420.0, 8.0),
        },
    ]
...
class RouteTimingChartTC(unittest.TestCase):
    ...
    def test_chart_compares_every_timed_candidate(self):
        entries = self.chart._canvas.entries()

        self.assertIn('numpy', [entry['name'] for entry in entries])
        self.assertEqual(entries[1]['median'], 620.0)
        self.assertEqual(entries[1]['p95'], 680.0)
        self.assertTrue(entries[1]['auto'])
        self.assertTrue(entries[2]['winner'])

    def test_tooltip_reports_measurement_and_route_context(self):
        tooltip = self.chart._canvas.tooltip_for_row(1)

        self.assertIn('Median: 620.0 ns', tooltip)
        self.assertIn('P95: 680.0 ns', tooltip)
        self.assertIn('Noise (MAD / median): 5.000%', tooltip)
    ...
    def test_incorrect_or_unmeasured_routes_are_gray(self):
        entries = self.chart._canvas.entries()
        broken = next(entry for entry in entries
