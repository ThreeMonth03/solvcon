...
class BenchmarkResultsTC(unittest.TestCase):
    ...
    def test_chart_bar_and_whisker_positions(self):
        ...
        for width, y, median_x, p95_x in cases:
            with self.subTest(width=width, y=y):
                image = self.chart_image(width)
                for x, color in ((111, 'red'),
                                 (median_x - 3, 'red'),
                                 (median_x + 3, 'white'),
                                 (p95_x - 3, 'white'),
                                 (p95_x, 'black'),
                                 (p95_x + 3, 'white')):
                    self.assertEqual(image.pixelColor(x, y),
                                     QtGui.QColor(color), f'({x}, {y})')
