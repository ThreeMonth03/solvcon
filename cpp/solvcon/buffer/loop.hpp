 ...
 * one operand position along a broadcast axis. contiguous() constructs the
 * mapping for adjacent fixed-size blocks.
 *
...
public:
    ...
    static OperandMapping contiguous(LoopDomain const & domain, ssize_t block_size);
...
{
    return std::ranges::fold_left(m_shape, size_t{1}, std::multiplies<size_t>{});
}
...
inline OperandMapping OperandMapping::contiguous(LoopDomain const & domain, ssize_t block_size)
{
...
inline void MappedOffsetCursor::advance()
{
    for (size_t axis_plus_one = m_domain.rank(); axis_plus_one > 0; --axis_plus_one)
    {
        ...
        for (size_t operand = 0; operand < m_mappings.size(); ++operand)
        {
            m_offsets[operand] -= m_mappings[operand].stride(axis) * extent;
        }
    }
    m_valid = false;
}
