...
/// Identify operands in the batch offset mappings.
enum class MatmulOperand : std::uint8_t
{
    Output,
    Lhs,
    Rhs,
}; /* end enum class MatmulOperand */
...
void MatmulExecutor<Array>::execute(MatmulSelection const & selection)
{
    ...
    case MatmulKernel::FixedJki:
        execute_fixed<MatmulKernel::FixedJki>();
        return;
...
MatmulSelection MatmulExecutor<Array>::select_execution() const
{
    ...
    if constexpr (can_matmul_blas_v<value_type>)
    {
        ...
        if (selection.kernel != MatmulKernel::GenericIjk)
        {
            return selection;
        }
    }
...
void MatmulExecutor<Array>::execute_contractions()
{
    ...
    {
        execute_at<Kernel>(
            cursor.offset(MatmulOperand::Output),
            cursor.offset(MatmulOperand::Lhs),
            cursor.offset(MatmulOperand::Rhs));
    }
}

template <typename Array>
template <MatmulKernel Kernel>
void MatmulExecutor<Array>::execute_fixed()
{
    static_assert(Kernel == MatmulKernel::FixedIkj || Kernel == MatmulKernel::FixedJki);
    if constexpr (can_matmul_fixed_v<value_type>)
    {
        if constexpr (Kernel == MatmulKernel::FixedIkj)
        {
            bool const lhs_inner_is_unit_stride = m_plan.lhs_inner_stride() == 1;
            execute_fixed_ikj(
                m_output_data, m_lhs_data, m_rhs_data, m_plan, lhs_inner_is_unit_stride);
        }
        else
        {
            execute_fixed_jki(m_output_data, m_lhs_data, m_rhs_data, m_plan);
        }
        return;
    }
    throw std::logic_error("MatmulExecutor::execute_fixed(): unavailable fixed kernel");
}
