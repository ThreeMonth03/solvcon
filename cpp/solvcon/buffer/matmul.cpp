...
template <typename T, size_t Side, size_t ColumnStart, size_t ColumnCount, bool LhsRowMajor>
void run_fixed_ikj_block(
    T * output,
    T const * lhs,
    T const * rhs,
    ...
    ssize_t rhs_inner_stride)
{
    ...
    for (ssize_t row = 0; row < side; ++row)
    {
        ...
        for (ssize_t inner = 0; inner < side; ++inner)
        {
            ...
            lhs_value_ptr += LhsRowMajor ? 1 : lhs_inner_stride;
...
template <typename T, size_t Side, bool LhsRowMajor>
void run_fixed_ikj_loop(
    T * output,
    T const * lhs,
    T const * rhs,
    ssize_t lhs_row_stride,
    ...
    ssize_t rhs_inner_stride)
{
    ...
    run_fixed_ikj_block<T, Side, 0, block_size, LhsRowMajor>(
        output, lhs, rhs, lhs_row_stride, lhs_inner_stride, rhs_inner_stride);
    if constexpr (Side > block_size)
    {
        run_fixed_ikj_block<T, Side, block_size, Side - block_size, LhsRowMajor>(
            output, lhs, rhs, lhs_row_stride, lhs_inner_stride, rhs_inner_stride);
    }
}
    ...
    bool lhs_is_row_major)
{
    ...
    {
    case 8:
        run_fixed_ikj<T, 8>(output, lhs, rhs, plan, lhs_is_row_major);
        return;
    case 9:
        run_fixed_ikj<T, 9>(output, lhs, rhs, plan, lhs_is_row_major);
        return;
    case 10:
        run_fixed_ikj<T, 10>(output, lhs, rhs, plan, lhs_is_row_major);
        return;
    case 11:
        run_fixed_ikj<T, 11>(output, lhs, rhs, plan, lhs_is_row_major);
        return;
    case 12:
        run_fixed_ikj<T, 12>(output, lhs, rhs, plan, lhs_is_row_major);
        return;
    case 13:
        run_fixed_ikj<T, 13>(output, lhs, rhs, plan, lhs_is_row_major);
        return;
    case 14:
        run_fixed_ikj<T, 14>(output, lhs, rhs, plan, lhs_is_row_major);
        return;
    case 15:
        run_fixed_ikj<T, 15>(output, lhs, rhs, plan, lhs_is_row_major);
        return;
...
void execute_fixed_ikj(
    ...
    bool lhs_is_row_major)
{
    dispatch_fixed_ikj(output, lhs, rhs, plan, lhs_is_row_major);
}
