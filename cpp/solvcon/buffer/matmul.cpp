    ...
    MatmulPlan const & plan)
{
    ...
    if constexpr (Side < static_cast<size_t>(FIXED_MATRIX_MAX_SIDE<T>))
    {
    ...
    MatmulPlan const & plan)
{
    ...
    case MatmulKernel::FixedIkj:
        dispatch_fixed_side<MatmulKernel::FixedIkj, T, static_cast<size_t>(FIXED_MATRIX_TUNING<T>.min_side)>(
            output, lhs, rhs, plan);
        return;
    case MatmulKernel::FixedJki:
        dispatch_fixed_side<MatmulKernel::FixedJki, T, static_cast<size_t>(FIXED_MATRIX_TUNING<T>.min_side)>(
            output, lhs, rhs, plan);
        return;
    default:
