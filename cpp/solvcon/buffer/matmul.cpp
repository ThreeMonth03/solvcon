...
{
    run_fixed_ikj_loop<T, Side, LhsInnerIsUnitStride>(
...
static void run_fixed_contraction(
    T * output,
    T const * lhs,
    T const * rhs,
    MatmulPlan const & plan)
{
    ...
    {
        run_fixed_ikj<T, Side, LhsInnerIsUnitStride>(output, lhs, rhs, plan);
    }
    else
    {
        static_assert(Kernel == MatmulKernel::FixedJki);
        run_fixed_jki<T, Side>(output, lhs, rhs, plan);
    }
}

template <MatmulKernel Kernel, typename T, size_t Side, bool LhsInnerIsUnitStride = false>
static void run_fixed_batches(
    T * output,
    T const * lhs,
    T const * rhs,
    MatmulPlan const & plan)
{
    ...
    {
        run_fixed_contraction<Kernel, T, Side, LhsInnerIsUnitStride>(output, lhs, rhs, plan);
        return;
    }

    for (MappedOffsetCursor cursor = plan.batch_cursor(); cursor; cursor.advance())
    {
        run_fixed_contraction<Kernel, T, Side, LhsInnerIsUnitStride>(
            output + cursor.offset(MatmulOperand::Output),
            lhs + cursor.offset(MatmulOperand::Lhs),
            rhs + cursor.offset(MatmulOperand::Rhs),
            plan);
    }
}

template <typename T, size_t Side>
static void run_fixed_ikj_side(
    T * output,
    T const * lhs,
    T const * rhs,
    MatmulPlan const & plan,
    bool lhs_inner_is_unit_stride)
{
    ...
    {
        run_fixed_batches<MatmulKernel::FixedIkj, T, Side, true>(output, lhs, rhs, plan);
    }
    else
    {
        run_fixed_batches<MatmulKernel::FixedIkj, T, Side, false>(output, lhs, rhs, plan);
    }
}
    ...
    bool lhs_inner_is_unit_stride)
{
    ...
    {
    case 8:
        run_fixed_ikj_side<T, 8>(output, lhs, rhs, plan, lhs_inner_is_unit_stride);
        return;
    case 9:
        run_fixed_ikj_side<T, 9>(output, lhs, rhs, plan, lhs_inner_is_unit_stride);
        return;
    case 10:
        run_fixed_ikj_side<T, 10>(output, lhs, rhs, plan, lhs_inner_is_unit_stride);
        return;
    case 11:
        run_fixed_ikj_side<T, 11>(output, lhs, rhs, plan, lhs_inner_is_unit_stride);
        return;
    case 12:
        run_fixed_ikj_side<T, 12>(output, lhs, rhs, plan, lhs_inner_is_unit_stride);
        return;
    case 13:
        run_fixed_ikj_side<T, 13>(output, lhs, rhs, plan, lhs_inner_is_unit_stride);
        return;
    case 14:
        run_fixed_ikj_side<T, 14>(output, lhs, rhs, plan, lhs_inner_is_unit_stride);
        return;
    case 15:
        run_fixed_ikj_side<T, 15>(output, lhs, rhs, plan, lhs_inner_is_unit_stride);
        return;
    ...
    MatmulPlan const & plan)
{
    ...
    {
    case 8:
        run_fixed_batches<MatmulKernel::FixedJki, T, 8>(output, lhs, rhs, plan);
        return;
    case 9:
        run_fixed_batches<MatmulKernel::FixedJki, T, 9>(output, lhs, rhs, plan);
        return;
    case 10:
        run_fixed_batches<MatmulKernel::FixedJki, T, 10>(output, lhs, rhs, plan);
        return;
    case 11:
        run_fixed_batches<MatmulKernel::FixedJki, T, 11>(output, lhs, rhs, plan);
        return;
    case 12:
        run_fixed_batches<MatmulKernel::FixedJki, T, 12>(output, lhs, rhs, plan);
        return;
    case 13:
        run_fixed_batches<MatmulKernel::FixedJki, T, 13>(output, lhs, rhs, plan);
        return;
    case 14:
        run_fixed_batches<MatmulKernel::FixedJki, T, 14>(output, lhs, rhs, plan);
        return;
    case 15:
        run_fixed_batches<MatmulKernel::FixedJki, T, 15>(output, lhs, rhs, plan);
        return;
