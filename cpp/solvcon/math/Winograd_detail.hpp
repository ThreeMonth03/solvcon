...
/**
 * @file
 * Define the internal one-level Winograd GEMM schedule.
 *
...
template <typename T, typename ProductExecutor>
void multiply(BlasGemmOperation<T> const & gemm, ProductExecutor const & execute_product);

/**
 * @brief Evaluate one Winograd decomposition.
 *
 * OneLevelExecutor divides one GEMM into quadrants, forms the Winograd input
 * blocks in two reusable scratch regions, and sends seven product descriptors
 * to the caller-provided multiplication callback. The products write or
 * accumulate directly into the four output quadrants. The callback must
 * consume each product synchronously because later products reuse the same
 * scratch.
 *
 * For an `8 x 12 x 16` contraction, OneLevelExecutor forms seven `4 x 6 x
 * 8` products and uses scratch for one `4 x 8` lhs block and one `8 x 6` rhs
 * block.
 *
 * @tparam T Element type.
 */
template <typename T>
class OneLevelExecutor
{
private:
    template <typename U, typename ProductExecutor>
    friend void multiply(BlasGemmOperation<U> const & gemm, ProductExecutor const & execute_product);

    explicit OneLevelExecutor(BlasGemmOperation<T> const & gemm);

    template <typename ProductExecutor>
    void execute(ProductExecutor const & execute_product);
    ...
    BlasMatrixView<T> lhs_scratch_view();
    BlasMatrixView<T> rhs_scratch_view();
    ...
    void multiply_into(
        ...
        T alpha,
        T beta,
        ProductExecutor const & execute_product);
...
}; /* end class OneLevelExecutor */

template <typename T>
OneLevelExecutor<T>::OneLevelExecutor(BlasGemmOperation<T> const & gemm)
...
template <typename T>
template <typename ProductExecutor>
void OneLevelExecutor<T>::execute(ProductExecutor const & execute_product)
{
    // P7 = (A11 - A21)(B22 - B12); initialize C21.
    form_lhs_scratch(m_a11, m_a21, T{-1});
    form_rhs_scratch(m_b22, m_b12, T{-1});
    multiply_into(lhs_scratch_view(), rhs_scratch_view(), m_c21, T{1}, T{0}, execute_product);

    // P5 = (A21 + A22)(B12 - B11); initialize C22.
    form_lhs_scratch(m_a21, m_a22, T{1});
    form_rhs_scratch(m_b12, m_b11, T{-1});
    multiply_into(lhs_scratch_view(), rhs_scratch_view(), m_c22, T{1}, T{0}, execute_product);

    // P6 = (A21 + A22 - A11)(B22 - B12 + B11); initialize C12.
    form_lhs_scratch(lhs_scratch_view(), m_a11, T{-1});
    form_rhs_scratch(m_b22, rhs_scratch_view(), T{-1});
    multiply_into(lhs_scratch_view(), rhs_scratch_view(), m_c12, T{1}, T{0}, execute_product);

    // P1 initializes C11; U2, U3, U4, and U7 share one output pass.
    multiply_into(m_a11, m_b11, m_c11, T{1}, T{0}, execute_product);
    form_output_intermediates();

    // P3 accumulates into U4 to form C12 = U5.
    form_lhs_scratch(m_a12, lhs_scratch_view(), T{-1});
    multiply_into(lhs_scratch_view(), m_b22, m_c12, T{1}, T{1}, execute_product);

    // P4 and P2 accumulate into U3 and P1 to form C21 = U6 and C11 = U1.
    form_rhs_scratch(rhs_scratch_view(), m_b21, T{-1});
    multiply_into(m_a22, rhs_scratch_view(), m_c21, T{-1}, T{1}, execute_product);
    multiply_into(m_a12, m_b21, m_c11, T{1}, T{1}, execute_product);
}

template <typename T>
BlasMatrixView<T> OneLevelExecutor<T>::lhs_scratch_view()
{
    return {
        .m_data = lhs_scratch(),
...
template <typename T>
BlasMatrixView<T> OneLevelExecutor<T>::rhs_scratch_view()
{
    return {
        .m_data = rhs_scratch(),
...
template <typename T>
void OneLevelExecutor<T>::form_lhs_scratch(BlasMatrixView<T> lhs, BlasMatrixView<T> rhs, T rhs_scale)
{
    combine_block(lhs, rhs, lhs_scratch(), m_block_rows, m_block_inner_size, rhs_scale);
}

template <typename T>
void OneLevelExecutor<T>::form_rhs_scratch(BlasMatrixView<T> lhs, BlasMatrixView<T> rhs, T rhs_scale)
{
...
template <typename T>
template <typename ProductExecutor>
void OneLevelExecutor<T>::multiply_into(
    BlasMatrixView<T> lhs,
    BlasMatrixView<T> rhs,
    BlasOutputView<T> output,
    T alpha,
    ...
    ProductExecutor const & execute_product)
{
    ...
    execute_product(product);
}

template <typename T>
void OneLevelExecutor<T>::form_output_intermediates()
{
...
{
    validate(gemm);
    OneLevelExecutor<T> executor(gemm);
    executor.execute(execute_product);
}
