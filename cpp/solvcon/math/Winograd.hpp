 ...
 *
 * @see https://doi.org/10.1016/0024-3795(71)90009-7
 * @ingroup group_core
 */
...
void multiply(Gemm<T> const & gemm, Multiply const & multiply_product);

/**
 * @brief Evaluate one level of Winograd matrix multiplication.
 *
 * Kernel divides one GEMM into quadrants, forms the Winograd input blocks in
 * two reusable scratch regions, and sends seven product descriptors to the
 ...
 *
 * For an `8 x 12 x 16` contraction, Kernel forms seven `4 x 6 x 8` products
...
private:
    ...
    friend void multiply(Gemm<U> const & gemm, Multiply const & multiply_product);

    explicit Kernel(Gemm<T> const & gemm);

    template <typename Multiply>
    void evaluate(Multiply const & multiply);

    size_t lhs_size() const { return static_cast<size_t>(m_rows) * static_cast<size_t>(m_inner_size); }
    size_t rhs_size() const { return static_cast<size_t>(m_inner_size) * static_cast<size_t>(m_columns); }
    T * lhs_scratch() { return m_scratch.get(); }
    T * rhs_scratch() { return m_scratch.get() + lhs_size(); }
...
template <typename T>
Kernel<T>::Kernel(Gemm<T> const & gemm)
    : m_rows(gemm.rows / 2)
    , m_columns(gemm.columns / 2)
    , m_inner_size(gemm.inner_size / 2)
    , m_a11(make_subview(gemm.lhs, 0, 0))
    , m_a12(make_subview(gemm.lhs, 0, m_inner_size))
    , m_a21(make_subview(gemm.lhs, m_rows, 0))
    , m_a22(make_subview(gemm.lhs, m_rows, m_inner_size))
    , m_b11(make_subview(gemm.rhs, 0, 0))
    , m_b12(make_subview(gemm.rhs, 0, m_columns))
    , m_b21(make_subview(gemm.rhs, m_inner_size, 0))
    , m_b22(make_subview(gemm.rhs, m_inner_size, m_columns))
    , m_c11(make_subview(gemm.output, 0, 0))
    , m_c12(make_subview(gemm.output, 0, m_columns))
    , m_c21(make_subview(gemm.output, m_rows, 0))
    , m_c22(make_subview(gemm.output, m_rows, m_columns))
    , m_scratch(std::make_unique_for_overwrite<T[]>(lhs_size() + rhs_size())) // NOLINT(cppcoreguidelines-avoid-c-arrays,modernize-avoid-c-arrays)
{
}

template <typename T>
template <typename Multiply>
void Kernel<T>::evaluate(Multiply const & multiply)
{
...
template <typename T>
BlasMatrixView<T> Kernel<T>::lhs_block()
{
    return {
        .m_data = lhs_scratch(),
        .m_leading_dimension = m_inner_size,
        .m_transpose = BlasTranspose::None,
    };
}

template <typename T>
BlasMatrixView<T> Kernel<T>::rhs_block()
{
    return {
        .m_data = rhs_scratch(),
        .m_leading_dimension = m_columns,
        .m_transpose = BlasTranspose::None,
    };
}

template <typename T>
void Kernel<T>::form_lhs_scratch(BlasMatrixView<T> lhs, BlasMatrixView<T> rhs, T rhs_scale)
{
    combine_block(lhs, rhs, lhs_scratch(), m_rows, m_inner_size, rhs_scale);
}

template <typename T>
void Kernel<T>::form_rhs_scratch(BlasMatrixView<T> lhs, BlasMatrixView<T> rhs, T rhs_scale)
{
    combine_block(lhs, rhs, rhs_scratch(), m_inner_size, m_columns, rhs_scale);
}

template <typename T>
template <typename Multiply>
void Kernel<T>::multiply_into(
    BlasMatrixView<T> lhs,
    BlasMatrixView<T> rhs,
    BlasOutputView<T> output,
    T alpha,
...
template <typename T>
void Kernel<T>::form_output_intermediates()
{
    for (ssize_t row = 0; row < m_rows; ++row)
    {
        ...
        for (ssize_t column = 0; column < m_columns; ++column)
        {
...
{
    validate(gemm);
    Kernel<T> kernel(gemm);
    kernel.evaluate(multiply_product);
}
    ...
    BlasOutputView<T> output)
{
    ...
    auto const multiply_product = [](winograd::Gemm<T> const & product)
    {
        gemm_blas(
            product.rows,
            product.columns,
