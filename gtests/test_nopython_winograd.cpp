...
template <typename T>
void reference_gemm(Gemm<T> const & gemm)
{
    ...
    std::vector<T> const & rhs)
{
    ...
    Gemm<T> const gemm = make_gemm(rows, columns, inner_size, lhs.data(), rhs.data(), output.data());
    Gemm<T> reference = gemm;
    reference.output.m_data = expected.data();
    reference_gemm(reference);
    size_t product_calls = 0;

    auto const multiply_product = [&product_calls](Gemm<T> const & product)
    {
        ++product_calls;
...
void check_winograd_padded_output(ssize_t rows, ssize_t columns, ssize_t inner_size)
{
    ...
    Gemm<T> gemm = make_gemm(rows, columns, inner_size, lhs.data(), rhs.data(), output.data());
    gemm.output.m_leading_dimension = output_stride;
    Gemm<T> reference = make_gemm(rows, columns, inner_size, lhs.data(), rhs.data(), expected.data());
    reference_gemm(reference);

    auto const multiply_product = [](Gemm<T> const & product)
    { reference_gemm(product); };
    multiply(gemm, multiply_product);
...
TEST(WinogradKernel, rejects_invalid_gemm)
{
    ...
    Gemm<double> gemm = make_gemm(8, 12, 16, lhs.data(), rhs.data(), output.data());
    auto const multiply_product = [](Gemm<double> const &) {};
...
TEST(WinogradKernel, blas_products)
{
    ...
    std::vector<double> rhs(192);
    std::vector<double> output(96);
    Gemm<double> const gemm = make_gemm(8, 12, 16, lhs.data(), rhs.data(), output.data());
    EXPECT_THAT(
        [&]
