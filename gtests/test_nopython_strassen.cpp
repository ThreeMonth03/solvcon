...
{
    for (ssize_t row = 0; row < gemm.m_rows; ++row)
    {
        for (ssize_t column = 0; column < gemm.m_columns; ++column)
        {
            T total{};
            for (ssize_t inner = 0; inner < gemm.m_inner_size; ++inner)
            {
                total += gemm.m_lhs.m_data[row * gemm.m_lhs.m_leading_dimension + inner] *
                         gemm.m_rhs.m_data[inner * gemm.m_rhs.m_leading_dimension + column];
            }
            gemm.m_output[row * gemm.m_columns + column] = total;
        }
    }
}
...
template <typename T>
size_t run_strassen(
    ssize_t rows, ssize_t columns, ssize_t inner_size, size_t depth, strassen::Workspace<T> & workspace)
{
    std::vector<T> lhs;
    std::vector<T> rhs;
    ...
    expected_gemm.m_output = expected.data();
...
template <typename T>
void check_depth(size_t depth, size_t expected_leaf_calls)
{
    strassen::Workspace<T> workspace;
    EXPECT_EQ(run_strassen<T>(8, 12, 16, depth, workspace), expected_leaf_calls);
}
...
TEST(StrassenKernel, matches_reference_at_each_depth)
{
    ...
    EXPECT_EQ(run_strassen<double>(3, 5, 7, 0, workspace), 1);
...
TEST(StrassenKernel, rejects_invalid_gemm)
{
    ...
    gemm.m_rows = 0;
    EXPECT_THROW(strassen::multiply(gemm, 1, workspace, leaf), std::invalid_argument);
    gemm.m_rows = 7;
    EXPECT_THROW(strassen::multiply(gemm, 1, workspace, leaf), std::invalid_argument);
    gemm.m_rows = 8;
    gemm.m_columns = 10;
    EXPECT_THROW(strassen::multiply(gemm, 2, workspace, leaf), std::invalid_argument);
    gemm.m_columns = 12;
    gemm.m_lhs.m_transpose = solvcon::BlasTranspose::Transpose;
    EXPECT_THROW(strassen::multiply(gemm, 1, workspace, leaf), std::invalid_argument);
    gemm.m_lhs.m_transpose = solvcon::BlasTranspose::None;
    gemm.m_lhs.m_leading_dimension = 15;
    EXPECT_THROW(strassen::multiply(gemm, 1, workspace, leaf), std::invalid_argument);
}
...
{
    strassen::Workspace<double> workspace;
    EXPECT_EQ(run_strassen<double>(8, 12, 16, 2, workspace), 49);
    size_t const capacity = workspace.capacity();
    EXPECT_EQ(run_strassen<double>(6, 10, 8, 1, workspace), 7);
...
TEST(StrassenKernel, blas_leaf)
{
    ...
    detail::gemm_strassen(gemm, 0, workspace);
    EXPECT_EQ(output, 6);
#else
    EXPECT_THROW(detail::gemm_strassen(gemm, 0, workspace), std::runtime_error);
#endif
}
